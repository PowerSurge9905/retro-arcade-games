...
@export var speed = 500 + (25 * (GameManager.level - 1))

# Detects if the player is using the left/right arrow keys
# Checks if paddleCanMove in game_manager.gd is true
func _process(delta):
	var move_direction = 0
	if (Input.is_action_pressed("ui_left") || $"../Ball".position.x < (position.x - 20)) && GameManager.paddleCanMove:
		move_direction = -1
	elif (Input.is_action_pressed("ui_right") || $"../Ball".position.x > (position.x + 20)) && GameManager.paddleCanMove:
		move_direction = 1
